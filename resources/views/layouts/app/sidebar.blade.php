...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    <flux:sidebar sticky collapsible="mobile"
        ...
        <flux:sidebar.nav>
            ...
            <flux:sidebar.group heading="Control de Acceso" expandable icon="arrow-right-circle"
                :expanded="request()->routeIs('accesos*')">
                <flux:sidebar.item icon="arrow-right-circle" href="#"
                    :current="request()->routeIs('accesos.entrada*')" wire:navigate>
                    {{ __('Registrar Entrada') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="arrow-left-circle" href="#"
                    :current="request()->routeIs('accesos.salida*')" wire:navigate>
                    {{ __('Registrar Salida') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="clipboard-document-list" href="#"
                    :current="request()->routeIs('accesos.historial*')" wire:navigate>
                    {{ __('Historial de Accesos') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="clock" href="#" :current="request()->routeIs('accesos.en-curso*')"
                    wire:navigate badge="24">
                    {{ __('Accesos en Curso') }}
                </flux:sidebar.item>
            </flux:sidebar.group>

            <!-- Módulo de Monitoreo (Expandible) -->
            <flux:sidebar.group heading="Monitoreo" expandable icon="eye"
                :expanded="request()->routeIs('monitoreo*')">
                <flux:sidebar.item icon="eye" href="#" :current="request()->routeIs('monitoreo.ocupacion*')"
                    wire:navigate>
                    {{ __('Ocupación Actual') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="chart-bar" href="#"
                    :current="request()->routeIs('monitoreo.estadisticas*')" wire:navigate>
                    {{ __('Estadísticas en Vivo') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="map" href="#" :current="request()->routeIs('monitoreo.mapa*')"
                    wire:navigate>
                    {{ __('Mapa de Ocupación') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="bell" href="#" :current="request()->routeIs('monitoreo.alertas*')"
                    wire:navigate badge="3">
                    {{ __('Alertas') }}
                </flux:sidebar.item>
            </flux:sidebar.group>
            ...
            <flux:sidebar.group heading="Reportes" expandable icon="document-text"
                ...
                <flux:sidebar.item icon="document-text" href="#"
                    :current="request()->routeIs('reportes.diarios*')" wire:navigate>
                    {{ __('Reporte Diario') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="calendar" href="#" :current="request()->routeIs('reportes.mensuales*')"
                    wire:navigate>
                    {{ __('Reporte Mensual') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="chart-pie" href="#"
                    :current="request()->routeIs('reportes.estadisticas*')" wire:navigate>
                    {{ __('Estadísticas') }}
                </flux:sidebar.item>
            ...
            <flux:sidebar.group heading="Seguridad" expandable icon="shield-check"
                :expanded="request()->routeIs('seguridad*') || request()->routeIs('lista-negra*') || request()->routeIs('incidencias*')">
                <flux:sidebar.item icon="no-symbol" href="#" :current="request()->routeIs('lista-negra*')"
                    wire:navigate>
                    {{ __('Lista Negra') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="exclamation-circle" href="#"
                    :current="request()->routeIs('incidencias*')" wire:navigate badge="5">
                    {{ __('Incidencias') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="shield-check" href="#" :current="request()->routeIs('auditoria*')"
                    wire:navigate>
                    {{ __('Auditoría') }}
                </flux:sidebar.item>
            </flux:sidebar.group>
            ...
            <flux:sidebar.group heading="Visitantes" expandable icon="user-group"
                :expanded="request()->routeIs('visitantes*')">
                <flux:sidebar.item icon="user-plus" href="#"
                    :current="request()->routeIs('visitantes.registrar*')" wire:navigate>
                    {{ __('Registrar Visitante') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="users" href="#" :current="request()->routeIs('visitantes.activos*')"
                    wire:navigate badge="8">
                    {{ __('Visitantes Activos') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="clock" href="#" :current="request()->routeIs('visitantes.historial*')"
                    wire:navigate>
            ...
            @if (auth()->user()->tieneAlgunRol(['Super Administrador', 'Administrador']))
                <flux:sidebar.group heading="Administración" expandable icon="cog-6-tooth"
                    :expanded="request()->routeIs('admin*')">
                    <flux:sidebar.item icon="adjustments-vertical" href="{{ route('admin.parametros') }}"
                        :current="request()->routeIs('admin.parametros*')" wire:navigate>
                        {{ __('Parámetros del Sistema') }}
                    </flux:sidebar.item>
                    <flux:sidebar.item icon="circle-stack" href="{{ route('admin.respaldos') }}"
                        :current="request()->routeIs('admin.respaldos*')" wire:navigate>
